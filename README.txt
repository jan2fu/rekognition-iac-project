...
1. Amazon Rekognition 
    ...
    - Detects objects (ex: "car," "dog") in images uploaded to S3.
2. AWS Lambda 
...
4. AWS DynamoDB	
    - NoSQL database to store Rekognition results.
    - Table ImageResults with image_name (PK) and labels (attribute).
5. AWS IAM 
...
Supporting Tools
    ...
    - CloudWatch: Logs Lambda output (ex: Rekognition analysis results).
...
Why Rekognition?
    ...
    - Cost-Effective: Pay per API call vs. maintaining a custom model (ex: SageMaker endpoints).
    - Example: Detecting inappropriate content in user-uploaded images (ex: social media moderation).

Terraform vs. AWS CloudFormation
Why Terraform?
    - Multi-Cloud Flexibility: Terraform supports AWS, Azure, etc. (CloudFormation is AWS-only).
    - Community Modules: Reusable modules for common patterns (ex: Lambda + S3).
...
Why Lambda?
    - Serverless Scaling: Automatically handles spikes in image uploads (ex: viral content).
...
Problem:
    - A social media platform needs to scan user-uploaded images for inappropriate content (ex: weapons, explicit material).
...
Solution:
    ...
    - Rekognition detects product labels (ex: "Coca-Cola 12-pack").
...
Solution:
    ...
    - Rekognition detects species (ex: "African elephant").
...
5. Future Enhancements:
    - Add Text Detection: Use Rekognition’s detect_text to extract text from images (ex: license plates).
